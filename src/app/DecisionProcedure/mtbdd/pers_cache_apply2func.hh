 ...
 *
 *  Description:
 *    Persistent cache binary Apply functor for OndriksMTBDD
 *
 *****************************************************************************/

#ifndef _VATA_PERS_CACHE_APPLY2FUNC_HH_
#define _VATA_PERS_CACHE_APPLY2FUNC_HH_
...
template <
	...
	typename DataOut
>
class VATA::MTBDDPkg::PersCacheApply2Functor
{
...
private:  // Private data members
	...
	static CacheHashTable ht;

private:  // Private methods

	PersCacheApply2Functor(const PersCacheApply2Functor&);
	PersCacheApply2Functor& operator=(const PersCacheApply2Functor&);
	...
	NodeOutPtrType recDescend(const Node1PtrType& node1, const Node2PtrType& node2)
	{
		...
		if (IsLeaf(node1) && GetDataFromLeaf(node1).empty())
		{
			ht.insert(std::make_pair(cacheAddress, node2));
			return node2;
		}

		if (IsLeaf(node2) && GetDataFromLeaf(node2).empty())
		{
			ht.insert(std::make_pair(cacheAddress, node1));
			return node1;
		}
...
template <
	...
	typename DataOut
>
typename VATA::MTBDDPkg::PersCacheApply2Functor<Base, Data1, Data2, DataOut>::CacheHashTable
	VATA::MTBDDPkg::PersCacheApply2Functor<Base, Data1, Data2, DataOut>::ht;
